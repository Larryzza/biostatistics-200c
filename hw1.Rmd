...
We selected `cbind(ncases, ncontrols) ~ unclass(agegp) + unclass(alcgp) + unclass(tobgp) + unclass(agegp):unclass(alcgp) + unclass(alcgp):unclass(tobgp)` as the best model. After drop1 test (chisq), we found that interaction of agegp and alcgp is not significant  within the 95% confidence interval. 
